...
const getDevice = async (req: RequestAndDevice, res: Response) => {
  try {
    ...
    const deviceSections: Model<IDeviceSection>[] =
      await DeviceSection.findAll();

    return res.status(200).json({
      total: alldevices.length,
      items: alldevices.map((device) => ({
        ...device.dataValues,
        deviceSection: deviceSections.filter(
          (deviceSection) =>
            deviceSection.dataValues.deviceId === device.dataValues.id
        ),
      })),
    });
  } catch (error) {
